...
<head>
    ...
    <style>
        ...
        @media print {
            ...
            /* ------------------------------------- */
            /* ضغط عناصر القوائم وإخفاؤها مبدئياً */
            /* ------------------------------------- */
            .employee-card, .document-card, .vehicle-card {
                /* إخفاء جميع الكروت افتراضياً */
                display: none !important; 
            ...
            /* إظهار الكروت المحددة للطباعة فقط (يعمل لـ "طباعة المحدد فقط") */
            ...
            /* إظهار كل الكروت في التبويب النشط فقط إذا كانت علامة 'print-all-active-tab' موجودة على الجسم */
            body.print-all-active-tab .tab-pane.active .employee-card,
            body.print-all-active-tab .tab-pane.active .document-card,
            body.print-all-active-tab .tab-pane.active .vehicle-card {
                 display: block !important;
            }
            
            /* ضغط العناصر المطبوعة */
            .card-body {
...
<body>
    ...
    <div class="document-container">
            ...
            <div class="tab-pane fade" id="residency-form" role="tabpanel" aria-labelledby="residency-form-tab">
                <div class="form-section-container" style="max-width: 800px; margin: 30px auto;">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6 form-section">
                            <label class="form-label">الرقم المدني / Civil ID</label>
                    ...
                    <div class="signature-section mt-5 pt-3 border-top">
                        <div class="row text-center">
                            <div class="col-md-6">
                                <p class="mb-1">توقيع الموظف:</p>
                                <p class="english-translation">(Employee Signature)</p>
                                <div style="height: 50px; border-bottom: 1px dashed #999; width: 80%; margin: auto;"></div>
                            </div>
                            <div class="col-md-6">
                                <p class="mb-1">توقيع المسؤول:</p>
                                <p class="english-translation">(Manager Signature)</p>
                                <div style="height: 50px; border-bottom: 1px dashed #999; width: 80%; margin: auto;"></div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="no-print mt-4 text-center print-controls">
            ...
            <button onclick="printActivePage()" class="btn btn-outline-primary me-2">
                <i class="bi bi-printer-fill"></i> طباعة التبويب النشط بالكامل
            </button>
    ...
    <script>
        ...
        function togglePrintSelection(type, index) {
            ...
            saveData(type);
            // لا نحتاج لإعادة العرض بالكامل، يمكننا تحديث الـ class مباشرة
            const card = document.querySelector(`.${DATA_MAP[type].listClass}[data-index="${index}"]`);
            if (card) {
                card.classList.toggle('selected-for-print', list[index].printSelected);
                card.classList.toggle('print-selected', list[index].printSelected);
            }
        }
        ...
        /**
         * الوظيفة الجديدة لطباعة المحدد فقط
         * تعتمد على أن الكروت غير المحددة مخفية في CSS
         */
        function printSelectedItems() {
            ...
            if (type === 'none' || activeTabId === 'residency-form') {
                // النموذج الفردي أو أي تبويب غير قائمة يُطبع كاملاً
                window.print();
                return;
            }

            const list = getListData(type);
            let hasSelected = list.some(item => item.printSelected);

            if (!hasSelected) {
                 alert('الرجاء تحديد بند واحد على الأقل للطباعة.');
                 return;
            }

            // فقط قم بالطباعة، و CSS سيتولى إظهار المحدد فقط وإخفاء غير المحدد
            window.print();
        }
        
        /**
         * الوظيفة الجديدة لطباعة التبويب النشط بالكامل (حتى غير المحدد)
         * تضيف class مؤقت للجسم لتجاوز إخفاء العناصر غير المحددة في CSS
         */
        function printActivePage() {
            // هذا الزر يقوم بطباعة كامل محتوى التبويب النشط
            
            // تحقق إذا كان التبويب عبارة عن قائمة
            const activeTabId = document.querySelector('.tab-pane.active')?.id;
            if (activeTabId === 'residency-list' || activeTabId === 'documents' || activeTabId === 'vehicles') {
                document.body.classList.add('print-all-active-tab');
            }
            
            window.print();
            
            // يجب إزالة الكلاس بعد انتهاء الطباعة لإعادة الواجهة إلى وضعها الطبيعي
            setTimeout(() => {
                document.body.classList.remove('print-all-active-tab');
            }, 500); // 500ms كافية لبدء عملية الطباعة
        }
